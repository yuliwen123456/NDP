# V10 proposal: Conditional combination; only after successful independent V08 and V09

Version v10; parent V06; date 2026-09-14; branch feature/v10-conditional-combination.
Git commit: resolve the immutable proposal tag `proposal-v10` with `git rev-parse proposal-v10`; actual SHA is appended to main VERSION_HISTORY.md after commit.
Conclusion: NEEDS_MORE_TESTING. No algorithm implementation, training or evaluation has run.

Read main analysis/research_audit_v01.md for the full hypothesis, formula, duplication analysis, risks and acceptance protocol. Module planned only: No implementation path authorized yet.

All files here are NEW: README.md, config_v10_conditional_combination.json, metrics.json, train_command.sh, eval_command.sh, manifest_v10.json. Future implementation needs additional versioned files outside this frozen proposal directory (for example experiments/v10_conditional_combination_implementation01/); do not overwrite this proposal to implement it. This preserves the proposal and later implementation independently.
